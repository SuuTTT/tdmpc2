...
EXP_NAME=iter7_acrobot_ib_m1_steps400000
...
RESULT_CSV="$RESULT_DIR/acrobot-swingup_ib_abstraction.csv"
COMPARE_CSV="$RESULT_DIR/acrobot-swingup_ib_abstraction_compare.csv"
...
python3 scripts/export_acrobot_compile_result.py \
  ...
  --state-json .autosota_scheduler_test/iter7_acrobot_ib_abstraction_state.json \
  --run-log "$RUN_LOG" \
  --checkpoint "logs/${TASK}/${SEED}/${EXP_NAME}/models/final.pt" \
  --task "$TASK" \
  --iteration 7 \
  --mode information_bottleneck_abstraction \
  --idea-id IDEA-008 \
  --notes "Acrobot-swingup information-bottleneck abstraction probe using KL-to-uniform SimNorm latent regularization." \
  --seed "$SEED" \
