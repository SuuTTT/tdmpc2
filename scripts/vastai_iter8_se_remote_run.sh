...
if [[ ! -d "$REPO/.git" ]]; then
  git clone https://github.com/SuuTTT/tdmpc2.git "$REPO"
fi
cd "$REPO"
git fetch origin
...
run = wandb.init(project=project, entity=None, name="vastai_iter8_acrobot_se_export", job_type="export")
paths = [
