...
class TDMPC2(torch.nn.Module):
	...
	def _td_target(self, next_z, reward, terminated, task):
		...
		Args:
			next_z (torch.Tensor): Latent state at the next time step.
			reward (torch.Tensor): Reward at the current time step.
			terminated (torch.Tensor): Termination flag at the current time step.
			task (torch.Tensor): Task index (only used for multi-task experiments).

		Returns:
			torch.Tensor: TD-target at the current time step.
		"""
	...
	def _latent_ib_loss(self, z):
		"""
		KL-to-uniform information bottleneck loss over SimNorm symbols.
		Modules are SimNorm symbols aggregated across groups.
		"""
		z = z.clamp_min(1e-8)
	...
	@torch.no_grad()
	def diagnostics(self, buffer):
		"""
		Compute diagnostic metrics and visualizations.
		"""
		...
		# Transition flow
		z = self.model.encode(obs, task)
		assignments = z.view(z.shape[0], z.shape[1], -1, self.cfg.simnorm_dim).mean(-2)
		source = assignments[:-1].reshape(-1, self.cfg.simnorm_dim)
		target = assignments[1:].reshape(-1, self.cfg.simnorm_dim)
		flow = source.transpose(0, 1).matmul(target) / source.shape[0]
		flow = flow.cpu().numpy()
